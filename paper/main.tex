...
\begin{figure}[t]
    ...
    \caption{Deadline-aware capacity versus ordinary stability for \(C_{\min}=0.95\). A stable system can still fail a deadline-constrained EHC target.}
    \label{fig:frontier}
\end{figure}
At normalized deadline \(d=4\), the largest offered loads meeting a 95\% timely-completion target are approximately \(A^*=0.25,1.07,2.94,6.84\) for \(M=1,2,4,8\), respectively, all strictly below the corresponding stability limits \(A<M\). The gap is largest when deadlines are tight.
